...
Start Docker services in **both development and test modes** (never production) before verifying startup or shutdown behavior. Follow the setup instructions in `docs/11-guides/docker.md`. Confirm all required containers are in `running` or `healthy` state before proceeding. If the environment cannot be started, document why and skip dependent steps.
