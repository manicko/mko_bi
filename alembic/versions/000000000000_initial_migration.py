"""Consolidated initial migration - creates all tables according to ORM models.
...
def upgrade() -> None:
    """Create all tables from scratch according to current ORM models."""
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_dashboards_layout_id ON dashboards (layout_id)"
    )
    op.execute(
    ...
    op.execute(
        ...
        CREATE TABLE IF NOT EXISTS dashboard_access (
            ...
            dashboard_id UUID NOT NULL REFERENCES dashboards(id) ON DELETE CASCADE,
            permission dashboard_permission_level NOT NULL DEFAULT 'view'::dashboard_permission_level,
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_aggregated_data_dashboard_graph ON aggregated_data (dashboard_id, graph_id)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_aggregated_data_dims_gin ON aggregated_data USING GIN (dims)"
    )
    op.execute(
        "CREATE UNIQUE INDEX IF NOT EXISTS uq_aggregated_data_dashboard_graph_dims ON aggregated_data (dashboard_id, graph_id, (dims::text))"
    )
    ...
    # FK index (matching ORM __table_args__)
    op.execute(
...
def downgrade() -> None:
    ...
    op.execute("DROP INDEX IF EXISTS idx_dashboards_layout_id")
    op.execute("DROP INDEX IF EXISTS idx_dashboards_created_by")
    op.execute("DROP INDEX IF EXISTS idx_registration_requests_reviewed_by")
