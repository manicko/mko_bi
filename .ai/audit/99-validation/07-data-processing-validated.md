...
### DP-004 CONFIRMED: Missing validation for processing config field types

**Type:** BEST-PRACTICE (advisory)

**Finding:** The `_validate_settings` method validates field presence but does not validate field types. Processing settings accept arbitrary types in `ProcessingSettingsDict` (TypedDict), but invalid types for critical fields could cause silent failures or confusing errors during processing.

**Evidence:** The code only validates that settings is a dict and has required fields, but doesn't validate types or values for `separator`, `decimal_separator`, `column_types`, `date_format`.

### DP-005 CONFIRMED: Orphaned temp file risk on processing log failure

**Type:** BEST-PRACTICE (advisory)

**Finding:** In `process_upload_with_session`, when the file is moved to final location via `file_path.replace(final_file_path)` and then `enqueue_processing_job` fails, the moved file is cleaned up (line 263). However, if `db.commit()` at line 268 fails after successful enqueue, the file remains orphaned.

**Evidence:** Code path: file move → enqueue → commit. Cleanup only on enqueue failure, not on commit failure after enqueue succeeds.
...
|------------|---------------|------------------|--------|
| DP-001 | SPEC-DEVIATION | APPROVED (confirmed) | Validation requires fields (`loader`, `date_column`, `timezone`) that are documented but never used in processing pipeline |
| DP-002 | BEST-PRACTICE | SPEC-DEVIATION | Documented capability (`metric_agg` configurable) exists but value is never wired from config to aggregation function |
