...
def build_bar_chart(
    data: Dict[str, Any], x: str, y: str, title: Optional[str] = None
) -> go.Figure:
    ...
    """
    fig = go.Figure(data=[go.Bar(x=data[x], y=data[y], name=y)])

    if title:
        fig.update_layout(title=title)

    fig.update_layout(xaxis_title=x, yaxis_title=y, showlegend=True)
