...
from mko_b_i.models.data import (
    AggregatedData,
    DataFilter,
)
from mko_b_i.services.data_service import (
...
@router.get(
    ...
    description="Возвращает агрегированные данные для дашборда с применением фильтров.",
    ...
    try:
        ...
        if filters:
            try:
                ...
                logger.warning("Некорректный JSON в фильтрах: %s", e)
                raise HTTPException(
                    status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                    detail=f"Некорректный JSON в фильтрах: {e}",
                ) from e
        ...
        if parsed_filters:
            ...
            _, result = await get_dashboard_aggregates(
                dashboard_id=dashboard_id,
                user_id=current_user.id,
                db=db,
            )
        ...
        logger.info(
            "Агрегированные данные получены: dashboard_id=%s, charts_count=%s",
            dashboard_id,
            len(result),
        )
        return result
        ...
        logger.error(
            "Ошибка при получении данных дашборда id=%s: %s",
            dashboard_id,
            e,
        )
        raise HTTPException(
